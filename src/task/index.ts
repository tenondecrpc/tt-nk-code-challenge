...
async function retrieveAll(req: Request, res: Response): Promise<any> {
    ...
    return res.status(200).send({ message: 'Retrieve all tasks successfully', tasks });
}

async function create(req: Request, res: Response): Promise<any> {
    const checkRequest = validationResult(req);
    if (!checkRequest.isEmpty()) {
        return res.status(400).send({ message: "Errors were found with the request data", errors: checkRequest.array()});
    }
    const body = req.body || {};
    const task = new Task();
    ...
    return res.status(200).send({ message: 'Create a new task successfully', id: task.id });
}
...
async function update(req: Request, res: Response): Promise<any> {
    ...
    return res.status(200).send({ message: 'Update a task successfully', task: { ...task, completed: task.completed } });
}
...
async function remove(req: Request, res: Response): Promise<any> {
    ...
    return res.status(200).send({ message: 'Remove a task successfully', id: params.id });
}
